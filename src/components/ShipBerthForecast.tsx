...
const normalizeShip = (ship: Partial<ShipForecast> & { cargaGeral?: string; bercoProgramado?: number }, index: number): ShipForecast => ({
  ...
  tipoCarga: ship.tipoCarga || ship.cargaGeral || 'Carga Geral', bercoProgramado: ship.bercoProgramado ? String(ship.bercoProgramado).padStart(2, '0') as Berth : null,
...
export default function ShipBerthForecast() {
  ...
  const loadShips = useCallback(async () => {
    ...
    try {
      ...
      setShips((data || []).map((row, index) => normalizeShip({ ...row, nomeNavio: row.nome_navio, tipoCarga: row.tipo_carga, bercoProgramado: row.berco_programado, agenciaMaritima: row.agencia_maritima, quantidadeToneladas: row.quantidade_toneladas, fonteDados: row.fonte_dados, velocidadeNos: row.velocidade_nos }, index)).filter((ship) => (!ship.bercoProgramado || BERTHS.includes(ship.bercoProgramado)) && !/container|porta[- ]?conteineres/i.test(ship.tipoCarga)));
  ...
  const syncMarineTraffic = async () => {
    ...
    try {
      ...
      }).filter((ship) => (!ship.berco_programado || BERTHS.includes(ship.berco_programado)) && !/container|porta[- ]?conteineres/i.test(ship.tipo_carga));
  ...
  return <section className="ship-forecast-module" aria-label="Painel de previsão e rastreamento de navios">
    <div className="module-card-header forecast-heading"><div><span className="module-kicker"><Ship size={13} /> Carga geral · lineup CIPP</span><h3 className="forecast-title">Previsão e rastreamento · Berços CIPP</h3><p>Registros sem berço ficam pendentes para preenchimento manual ou lineup posterior.</p></div><div className="forecast-actions"><button type="button" className="forecast-refresh" onClick={loadShips} disabled={loading}><RefreshCw size={14} className={loading ? 'animate-spin' : ''} /> Atualizar dados</button>{canWrite && <button type="button" className="forecast-refresh" onClick={syncMarineTraffic} disabled={syncing}><Signal size={14} className={syncing ? 'animate-pulse' : ''} /> Sincronizar AIS</button>}</div></div>
